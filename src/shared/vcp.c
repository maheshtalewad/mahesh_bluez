...
struct bt_aics *pts_aics;  /* PTS AICS */
struct bt_vcp *pts_vcp;	/* PTS AICS */
							...
							struct iovec *iov)
{
	...
	audipst->chg_counter = -~audipst->chg_counter; /*Increment Change Counter*/
	gatt_db_attribute_notify(vdb->aics->aud_ip_state, (void *)audipst,
				sizeof(struct aud_ip_st),
				bt_vcp_get_att(vcp));
	ret = 0;
						...
						struct bt_vcp *vcp, struct iovec *iov)
{
	...
	if(audipst->gain_mode == AICS_GAIN_MODE_AUTO) {
		...
		ret = 0;
			...
			DBG(vcp, "Error!! Gain mode field value is not Automatic");
			ret = BT_ATT_AICS_ERROR_GAIN_MODE_CHANGE_NOT_ALLOWED;
	}
...
struct aics_op_handler {
		...
		AICS_OP("Set Gain Setting", BT_AICS_SET_GAIN_SETTING,
				sizeof(struct bt_aics_set_gain_setting), aics_set_gain_setting),
...
static struct bt_vcs *vcs_new(struct gatt_db *db, struct bt_vcp_db *vdb)
{
	...
	vcs->service = gatt_db_add_service(db, &uuid, true, VCS_TOTAL_NUMB_HANDLES);
...
static struct bt_aics *aics_new(struct gatt_db *db)
{
	...
	aics->service = gatt_db_add_service(db, &uuid, false, AICS_TOTAL_NUM_HANDLES);
	...
	aics->aud_ip_dscrptn = gatt_db_service_add_characteristic(aics->service,
							...
							BT_GATT_CHRC_PROP_NOTIFY,
							aics_input_descr_read, aics_input_descr_write,
							aics);
					 ...
					 void *user_data)
{
	...
	if (!success) {
		DBG(vcp, "Unable to read AICS Gain Setting Properties Char: error 0x%02x",
			att_ecode);
		return;
	}

	aics_gain_setting_prop = iov_pull_mem(&iov, sizeof(*aics_gain_setting_prop));
	if (!aics_gain_setting_prop) {
		DBG(vcp, "Unable to get AICS Gain Setting Properties Char");
		return;
	}

	DBG(vcp, "Gain Setting Properties, Units: %x", aics_gain_setting_prop->gain_setting_units);
	DBG(vcp, "Gain Setting Properties,  Min Value: %d", aics_gain_setting_prop->gain_setting_min);
	DBG(vcp, "Gain Setting Properties,  Max Value: %d", aics_gain_setting_prop->gain_setting_max);
}
					 ...
					 void *user_data)
{
	...
	if (!success) {
		DBG(vcp, "Unable to read AICS Audio Input Type Char: error 0x%02x",
			att_ecode);
		return;
	}
					 ...
					 void *user_data)
{
	...
	if (!success) {
		DBG(vcp, "Unable to read AICS Audio Input Status Char: error 0x%02x",
			att_ecode);
		return;
	}
					   ...
					   void *user_data)
{
	...
	if (!success) {
		DBG(vcp, "Unable to read AICS Audio Input Description char: error 0x%02x",
			att_ecode);
		return;
	}
...
static void foreach_aics_char(struct gatt_db_attribute *attr, void *user_data)
{
	...
	pts_vcp = vcp; /* PTS AICS */
	if (!gatt_db_attribute_get_char_data(attr, NULL, &value_handle,
						NULL, NULL, &uuid))
		return;

	bt_uuid16_create(&uuid_ipstate, AICS_INPUT_STATE_CHAR_UUID);
	bt_uuid16_create(&uuid_gain_setting_prop, AICS_GAIN_SETTING_PROP_CHAR_UUID);
	...
	if (!bt_uuid_cmp(&uuid, &uuid_ipstate)) {
		DBG(vcp, "AICS Audio Input State Char found: handle 0x%04x", value_handle);
		...
		vcp_read_value(vcp, value_handle, read_aics_audio_ip_state, vcp);
	...
	if (!bt_uuid_cmp(&uuid, &uuid_gain_setting_prop)) {
		DBG(vcp, "AICS Gain Setting Properties Char found: handle 0x%04x",
			value_handle);
	...
	if (!bt_uuid_cmp(&uuid, &uuid_ip_status)) {
		DBG(vcp, "AICS Audio Input Status Char found: handle 0x%04x", value_handle);
		...
		vcp_read_value(vcp, value_handle, read_aics_audio_ip_status, vcp);
	...
	if (!bt_uuid_cmp(&uuid, &uuid_ip_decs)) {
		DBG(vcp, "AICS Audio Input Description Char found: handle 0x%04x",
			value_handle);
		...
		vcp_read_value(vcp, value_handle, read_aics_audio_ip_description,
				   vcp);
...
void pts_aics_write_mute_value(uint16_t value)
{
	struct aud_ip_st *audipst;

	audipst= vdb_get_audipst(pts_aics->vdb);
	audipst->mute = (uint8_t)value;
}

void pts_aics_write_gain_mode_val(uint16_t value)
{
	struct aud_ip_st *audipst;

	audipst= vdb_get_audipst(pts_aics->vdb);
	audipst->gain_mode = (uint8_t)value;
}


static void pts_aics_conf_auto_gain_mode(struct bt_aics *aics, struct bt_vcp *vcp, uint16_t value)
{
	struct aud_ip_st *audipst;

	audipst= vdb_get_audipst(aics->vdb);
	if (!audipst) {
		DBG(vcp, "error: Audio Input State value is not available");
		return;
	}
	...
	audipst->gain_mode = (uint8_t)value;
	audipst->chg_counter = -~audipst->chg_counter; /*Increment Change Counter*/
	gatt_db_attribute_notify(aics->aud_ip_state, (void *)audipst,
				sizeof(struct aud_ip_st), bt_vcp_get_att(vcp));
	return;
}

void pts_aics_conf_auto_gain_mode_field(uint16_t value)
{
	pts_aics_conf_auto_gain_mode(pts_aics, pts_vcp, value);
}
/* AICS PTS End */
