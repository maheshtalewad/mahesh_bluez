					...
					struct bt_att *att, void *user_data)
{
	...
	printf("gatt_notify_cb: GATT server sending notification: notify.value:%d notify->len:%d\n", (int )*(notify.value), notify.len);
	if(!bt_gatt_server_send_notification(data->server,
				notify.handle, notify.value,
				notify.len, false))
		printf("gatt_notify_cb: Failed to send notification\n");
}
...
#define DISCOVER_PRIM_SERV_NOTIF	IOV_DATA(0x10, 0x01, 0x00, 0xff, 0xff, 0x00, 0x28), \
							IOV_DATA(0x11, 0x06, 0x01, 0x00, 0x05, 0x00, 0x4d, 0x18), \
							IOV_DATA(0x10, 0x06, 0x00, 0xff, 0xff, 0x00, 0x28), \
...
#define MICS_FIND_BY_TYPE_VALUE \
	...
	IOV_DATA(0x07, 0x01, 0x00, 0x05, 0x00), \
	IOV_DATA(0x06, 0x06, 0x00, 0xff, 0xff, 0x00, 0x28, 0x4d, 0x18), \
	IOV_DATA(0x01, 0x06, 0x06, 0x00, 0x0a)
...
int main(int argc, char *argv[])
{
	...
	define_test("MICS/SR/SGGIT/SER/BV-01-C", test_server, NULL, MICS_SR_SGGIT_SER_BV_01_C);
	define_test("MICS/SR/SGGIT/CHA/BV-01-C", test_server, NULL, MICS_SR_SGGIT_CHA_BV_01_C);
	define_test("MICS/SR/SPE/BI-01-C", test_server, NULL, MICS_SR_SPE_BI_1_C);
	define_test("MICS/SR/SPE/BI-02-C", test_server, NULL, MICS_SR_SPE_BI_02_C);  
	define_test("MICS/SR/SPN/BV-01-C", test_server, NULL, MICS_SR_SPN_BV_01_C); 
