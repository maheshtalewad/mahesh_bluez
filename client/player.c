...
static void cmd_enable_disable_mute_state(int argc, char *argv[])
{
	...
	if (g_dbus_proxy_set_property_basic(mics_default_ctrl->proxy, "mics_state",
					DBUS_TYPE_BOOLEAN, &mute_state,
					generic_callback, str, g_free) == TRUE)
		return;
	g_free(str);
...
static void cmd_micp_discover_mute(int argc, char *argv[])
{
	...
	printf("cmd_micp_discover_mute\n");
	if (g_dbus_proxy_set_property_basic(mics_default_ctrl->proxy, "micp_disc",
					DBUS_TYPE_BOOLEAN, &mute_state,
					generic_callback, str, g_free) == TRUE)
		return;
	g_free(str);
...
static void cmd_enable_read_mute_state(int argc, char *argv[])
{
	...
	bt_shell_printf("cmd_enable_read_mute_state: %x\n", handle);
	if (g_dbus_proxy_set_property_basic(mics_default_ctrl->proxy, "micp_read_char",
...
static void cmd_enable_write_mute_state(int argc, char *argv[])
{
	...
	bt_shell_printf("cmd_enable_write_mute_state: %x\n", handle);
	if (g_dbus_proxy_set_property_basic(mics_default_ctrl->proxy, "micp_write_char",
					DBUS_TYPE_UINT16, &handle,
					generic_callback, NULL, NULL) == TRUE)
		return;	
